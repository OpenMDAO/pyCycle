...
class CombineCooling(ExplicitComponent):
...
class CoolingCalcs(ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        if T_gas < self.options['T_metal']:
            ...
            try:
                ...
                raise AnalysisError('bad flow values in {}; W: {}'.format(self.pathname, W_primary))
...
class Row(Group):
    ...
    def setup(self):
        ...
        consts = self.add_subsystem('consts', IndepVarComp()) # values that should not be changed ever
