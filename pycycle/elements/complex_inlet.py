...
class Calcs(om.ExplicitComponent):
    """
    Performs subsonic, supersonic, and hypsersonic inlet engineering calculations.
    """
    ...
    def setup(self):
        ...
        self.add_input('etaBase', val=1.0, desc='Ram recovery')
        ...
        self.declare_partials('Pt_out', ['Pt_in', 'etaBase', 'MN_in'])
        self.declare_partials('F_ram', ['V_in', 'W_in'])

    def compute(self, inputs, outputs):
        
        etaBase = inputs['etaBase']
        MN_in = inputs['MN_in']
        
        if MN_in < 1.0:
            outputs['Pt_out'] = inputs['Pt_in'] * etaBase
            
        elif MN_in >= 1.0 and MN_in < 5.0:
            outputs['Pt_out'] = inputs['Pt_in'] * etaBase *(1-(0.075*((MN_in-1)**1.35)))
            
        elif MN_in >= 5.0:
            outputs['Pt_out'] = inputs['Pt_in'] * 800/(MN_in**4 + 935)
    ...
    def compute_partials(self, inputs, J):
        ...
        if MN_in < 1.0:
            J['Pt_out', 'Pt_in'] = inputs['etaBase']
            J['Pt_out', 'etaBase'] = inputs['Pt_in']
            J['Pt_out', 'MN_in'] = 0
            
        elif MN_in >= 1.0 and MN_in < 5.0:
            J['Pt_out', 'Pt_in'] = inputs['etaBase']*(1-(0.075*((MN_in-1)**1.35)))
            J['Pt_out', 'etaBase'] = inputs['Pt_in'] * (1-(0.075*((MN_in-1)**1.35)))
            J['Pt_out', 'MN_in'] = -.10125 * inputs['etaBase'] * inputs['Pt_in'] * ((MN_in-1)**0.35)
            
        elif MN_in >= 5.0:
            J['Pt_out', 'Pt_in'] = 800/(MN_in**4 + 935)
            J['Pt_out', 'etaBase'] = 0
            J['Pt_out', 'MN_in'] = -(3200 * inputs['Pt_in'] * (MN_in**3) / ((MN_in**4 + 935)**2))

class ComplexInlet(om.Group):
    """
    ...
    def setup(self):
        ...
        self.add_subsystem('calcs_inlet', Calcs(),
                           promotes_inputs=['etaBase', ('MN_in', 'Fl_I:stat:MN'), ('Pt_in', 'Fl_I:tot:P'),
...
if __name__ == "__main__":
    ...
    p.model.set_input_defaults('Fl_I:stat:MN', 1.5, units=None)
    ...
    p.check_partials(compact_print = True, includes=['*calcs_inlet*'])
