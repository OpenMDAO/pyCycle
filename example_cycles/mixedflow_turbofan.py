...
class MixedFlowTurbofan(pyc.Cycle):
    ...
    def setup(self):
        ...
        self.pyc_add_element('inlet_duct', pyc.Duct(design=design, thermo_data=thermo_spec))
        # Fan Components - Split here for CFD integration Add a CFDStart Compomponent
        self.pyc_add_element('fan', pyc.Compressor(map_data=pyc.AXI5, design=design, thermo_data=thermo_spec,
        ...
        self.pyc_add_element('splitter_core_duct', pyc.Duct(design=design, thermo_data=thermo_spec))
        self.pyc_add_element('lpc', pyc.Compressor(map_data=pyc.LPCMap, design=design, thermo_data=thermo_spec,map_extrap=True),
                                             promotes_inputs=[('Nmech','LP_Nmech')])
        self.pyc_add_element('lpc_duct', pyc.Duct(design=design, thermo_data=thermo_spec))
        self.pyc_add_element('hpc', pyc.Compressor(map_data=pyc.HPCMap, design=design, thermo_data=thermo_spec,
        ...
        self.pyc_add_element('burner', pyc.Combustor(design=design,thermo_data=thermo_spec,
                                                fuel_type='Jet-A(g)'))
        self.pyc_add_element('hpt', pyc.Turbine(map_data=pyc.HPTMap, design=design, thermo_data=thermo_spec,
                                          bleed_names=['cool3'],map_extrap=True),promotes_inputs=[('Nmech','HP_Nmech')])
        self.pyc_add_element('hpt_duct', pyc.Duct(design=design, thermo_data=thermo_spec))
        self.pyc_add_element('lpt', pyc.Turbine(map_data=pyc.LPTMap, design=design, thermo_data=thermo_spec,
                                        bleed_names=['cool1'],map_extrap=True), promotes_inputs=[('Nmech','LP_Nmech')])
        self.pyc_add_element('lpt_duct', pyc.Duct(design=design, thermo_data=thermo_spec))
        # Bypass Components
        self.pyc_add_element('bypass_duct', pyc.Duct(design=design, thermo_data=thermo_spec))
        # Mixer component
        self.pyc_add_element('mixer', pyc.Mixer(design=design, designed_stream=1))
        self.pyc_add_element('mixer_duct', pyc.Duct(design=design, thermo_data=thermo_spec))
        # Afterburner Components
        self.pyc_add_element('afterburner', pyc.Combustor(design=design,thermo_data=thermo_spec, fuel_type='Jet-A(g)'))
        # End CFD HERE
        # Nozzle
        self.pyc_add_element('mixed_nozz', pyc.Nozzle(nozzType='CD', lossCoef='Cfg', thermo_data=thermo_spec))
