...
from openmdao.api import Group, BalanceComp, DirectSolver, BoundsEnforceLS, NewtonSolver

from pycycle.constants import AIR_MIX, AIR_FUEL_MIX
from pycycle.connect_flow import connect_flow
from pycycle.cea import species_data
from pycycle.elements.api import FlightConditions, Inlet, Compressor, Combustor, Turbine, Nozzle, Shaft, Performance
...
class Turboshaft(Group):
    ...
    def setup(self):
        ...
        self.add_subsystem('fc', FlightConditions(thermo_data=thermo_spec,
                                    elements=AIR_MIX))
        self.add_subsystem('inlet', Inlet(design=design, thermo_data=thermo_spec,
                                    elements=AIR_MIX))
        self.add_subsystem('comp', Compressor(map_data=AXI5, design=design,
                                    thermo_data=thermo_spec, elements=AIR_MIX,),
        ...
        self.add_subsystem('burner', Combustor(design=design,thermo_data=thermo_spec,
                                    ...
                                    fuel_type='JP-7'))
        self.add_subsystem('turb', Turbine(map_data=LPT2269, design=design,
                                    thermo_data=thermo_spec, elements=AIR_FUEL_MIX,),
                                    promotes_inputs=[('Nmech', 'HP_Nmech')])
        self.add_subsystem('pt', Turbine(map_data=LPT2269, design=design,
                                    thermo_data=thermo_spec, elements=AIR_FUEL_MIX,),
                                    promotes_inputs=[('Nmech', 'LP_Nmech')])
        self.add_subsystem('nozz', Nozzle(nozzType='CV', lossCoef='Cv',
                                    thermo_data=thermo_spec, elements=AIR_FUEL_MIX))
