...
from openmdao.api import ImplicitComponent, Group, IndepVarComp, BalanceComp, DirectSolver, \
    BoundsEnforceLS, ArmijoGoldsteinLS, NewtonSolver, NonlinearBlockGS, PetscKSP

from pycycle.constants import AIR_MIX, AIR_FUEL_MIX
from pycycle.connect_flow import connect_flow
from pycycle.cea import species_data
from pycycle.elements.api import FlightConditions, Inlet, Compressor, Splitter, Combustor, Turbine, Nozzle, Shaft, Duct, Performance, BleedOut
...
from pycycle.maps.axi5 import AXI5 as LPCmap
from pycycle.maps.axi3_2 import AXI3_2 as HPCmap
from pycycle.maps.hpt1269 import HPT1269 as HPTmap
from pycycle.maps.lpt2269 import LPT2269 as LPTmap


class PowerBalance(ImplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('pwr_net', val=0., units='hp', desc='Net power on the shaft')
        self.add_input('pwr_ref', val=1., units='hp', desc='Reference power')

        if design:
            self.add_output('indep', val=1.5, lower=1.001, upper=8., desc='Independent: typically either turbine PR')
        else:
            self.add_output('indep', val=1.5, lower=500.0, units='rpm', desc='Independent: typically shaft Nmech')

    def solve_nonlinear(self, inputs, outputs):
        pass

    def apply_nonlinear(self, inputs, outputs, resids):
        # print(self.pathname, outputs['indep'])
        resids['indep'] = inputs['pwr_net']/inputs['pwr_ref']  # drive net power on the shaft to 0

    def linearize(self, inputs, outputs, J):
        J['indep', 'pwr_net'] = 1./inputs['pwr_ref']
        J['indep', 'pwr_ref'] = -inputs['pwr_net']/inputs['pwr_ref']**2


class Turboshaft(Group):
    ...
    def setup(self):
        ...
        self.add_subsystem('lpc', Compressor(map_data=LPCmap, design=design, thermo_data=thermo_spec, elements=AIR_MIX,
                                        statics=statics),promotes_inputs=[('Nmech','IP_Nmech')])
        self.add_subsystem('icduct', Duct(design=design, thermo_data=thermo_spec, elements=AIR_MIX, statics=statics))
        self.add_subsystem('hpc_axi', Compressor(map_data=HPCmap, design=design, thermo_data=thermo_spec, elements=AIR_MIX,
                                        statics=statics),promotes_inputs=[('Nmech','HP_Nmech')])
        self.add_subsystem('bld25', BleedOut(design=design, statics=statics, bleed_names=['cool1','cool2']))
        self.add_subsystem('hpc_centri', Compressor(map_data=HPCmap, design=design, thermo_data=thermo_spec, elements=AIR_MIX,
                                        statics=statics),promotes_inputs=[('Nmech','HP_Nmech')])
        ...
        self.add_subsystem('hpt', Turbine(map_data=HPTmap, design=design, thermo_data=thermo_spec, elements=AIR_FUEL_MIX,
                                        bleed_names=['cool3','cool4'], statics=statics),promotes_inputs=[('Nmech','HP_Nmech')])
        self.add_subsystem('duct43', Duct(design=design, thermo_data=thermo_spec, elements=AIR_FUEL_MIX, statics=statics))
        self.add_subsystem('lpt', Turbine(map_data=LPTmap, design=design, thermo_data=thermo_spec, elements=AIR_FUEL_MIX,
                                        bleed_names=['cool1','cool2'], statics=statics),promotes_inputs=[('Nmech','IP_Nmech')])
        self.add_subsystem('itduct', Duct(design=design, thermo_data=thermo_spec, elements=AIR_FUEL_MIX, statics=statics))
        self.add_subsystem('pt', Turbine(map_data=LPTmap, design=design, thermo_data=thermo_spec, elements=AIR_FUEL_MIX,
                                        statics=statics),promotes_inputs=[('Nmech','LP_Nmech')])
        self.add_subsystem('duct12', Duct(design=design, thermo_data=thermo_spec, elements=AIR_FUEL_MIX, statics=statics))
        self.add_subsystem('nozzle', Nozzle(nozzType='CV', lossCoef='Cv', thermo_data=thermo_spec, elements=AIR_FUEL_MIX))

        self.add_subsystem('lp_shaft', Shaft(num_ports=1),promotes_inputs=[('Nmech','LP_Nmech')])
        self.add_subsystem('ip_shaft', Shaft(num_ports=2),promotes_inputs=[('Nmech','IP_Nmech')])
        self.add_subsystem('hp_shaft', Shaft(num_ports=3),promotes_inputs=[('Nmech','HP_Nmech')])
        ...
        if statics:
            ...
            connect_flow(self, 'icduct.Fl_O', 'hpc_axi.Fl_I', connect_stat=False)
            connect_flow(self, 'hpc_axi.Fl_O', 'bld25.Fl_I', connect_stat=False)
            connect_flow(self, 'bld25.Fl_O', 'hpc_centri.Fl_I', connect_stat=False)
            connect_flow(self, 'hpc_centri.Fl_O', 'bld3.Fl_I', connect_stat=False)
            ...
            connect_flow(self, 'duct12.Fl_O', 'nozzle.Fl_I', connect_stat=False)
...
if __name__ == "__main__":
    ...
    # DESIGN CASE
    prob.model.add_subsystem('DESIGN', Turboshaft(statics=True))
    ...
    # for pt in pts:
    #     ODpt = prob.model.add_subsystem(pt, Turboshaft(design=False, statics=OD_statics))
