...
from openmdao.api import DirectSolver, BoundsEnforceLS, NewtonSolver, ScipyKrylov, ArmijoGoldsteinLS
...
from pycycle.elements.api import FlightConditions, Inlet, Compressor, Splitter, Combustor, Turbine, Nozzle, Shaft, Duct, Performance, BleedOut, Gearbox, CombineCooling, TurbineCooling
...
class N3(Group):
    ...
    def setup(self):
        ...
        self.add_subsystem('fan', Compressor(map_data=FanMap, design=design, thermo_data=thermo_spec, elements=AIR_MIX, map_extrap=True,
                                        bleed_names=[], statics=statics), promotes_inputs=[('Nmech','Fan_Nmech')])
        self.add_subsystem('splitter', Splitter(design=design, thermo_data=thermo_spec, elements=AIR_MIX, statics=statics))
        self.add_subsystem('duct2', Duct(design=design, expMN=2.0, thermo_data=thermo_spec, elements=AIR_MIX, statics=statics))
        self.add_subsystem('lpc', Compressor(map_data=LPCmap, design=design, thermo_data=thermo_spec, elements=AIR_MIX, map_extrap=True,
                                        statics=statics),promotes_inputs=[('Nmech','LP_Nmech')])
        self.add_subsystem('bld25', BleedOut(design=design, statics=statics, bleed_names=['sbv']))
        self.add_subsystem('duct25', Duct(design=design, expMN=2.0, thermo_data=thermo_spec, elements=AIR_MIX, statics=statics))
        self.add_subsystem('hpc', Compressor(map_data=HPCmap, design=design, thermo_data=thermo_spec, elements=AIR_MIX, map_extrap=True,
                                        bleed_names=['bld_inlet','bld_exit','cust'], statics=statics),promotes_inputs=[('Nmech','HP_Nmech')])
        self.add_subsystem('bld3', BleedOut(design=design, statics=statics, bleed_names=['bld_inlet','bld_exit']))
        ...
        self.add_subsystem('hpt', Turbine(map_data=HPTmap, design=design, thermo_data=thermo_spec, elements=AIR_FUEL_MIX, map_extrap=True,
                                        bleed_names=['bld_inlet','bld_exit'], statics=statics),promotes_inputs=[('Nmech','HP_Nmech')])
        self.add_subsystem('duct45', Duct(design=design, expMN=2.0, thermo_data=thermo_spec, elements=AIR_FUEL_MIX, statics=statics))
        self.add_subsystem('lpt', Turbine(map_data=LPTmap, design=design, thermo_data=thermo_spec, elements=AIR_FUEL_MIX, map_extrap=True,
                                        bleed_names=['bld_inlet','bld_exit'], statics=statics),promotes_inputs=[('Nmech','LP_Nmech')])
        self.add_subsystem('duct5', Duct(design=design, expMN=2.0, thermo_data=thermo_spec, elements=AIR_FUEL_MIX, statics=statics))
        ...
        self.add_subsystem('duct17', Duct(design=design, expMN=2.0, thermo_data=thermo_spec, elements=AIR_MIX, statics=statics))
        self.add_subsystem('byp_nozz', Nozzle(nozzType='CV', lossCoef='Cv', thermo_data=thermo_spec, elements=AIR_MIX))

        self.add_subsystem('fan_shaft', Shaft(num_ports=2),promotes_inputs=[('Nmech','Fan_Nmech')])
        self.add_subsystem('gearbox', Gearbox(design=design),promotes_inputs=[('N_in','LP_Nmech'), ('N_out','Fan_Nmech')])
        ...
        self.add_subsystem('ext_ratio', ExecComp('ER = core_V_ideal * core_Cv / ( byp_V_ideal *  byp_Cv )',
        ...
        if design:
            ...
            self.add_subsystem('hpc_CS',
                    ExecComp('CS = Win *(pow(Tout/518.67,0.5)/(Pout/14.696))',
            ...
            self.add_subsystem('fan_dia', ExecComp('FanDia = 2.0*(area/(pi*(1.0-hub_tip**2.0)))**0.5',
            ...
            self.add_subsystem('opr_calc', ExecComp('OPR_simple = FPR*LPCPR*HPCPR',
        ...
        if cooling:
            self.add_subsystem('hpt_cooling', TurbineCooling(n_stages=2, thermo_data=species_data.janaf, T_metal=2460.))
            self.add_subsystem('hpt_chargable', CombineCooling(n_ins=3))

            connect_flow(self, 'bld3.bld_inlet', 'hpt_cooling.Fl_cool', connect_stat=False)
            connect_flow(self, 'burner.Fl_O', 'hpt_cooling.Fl_turb_I')
            connect_flow(self, 'hpt.Fl_O', 'hpt_cooling.Fl_turb_O')
        ...
        #     self.set_order(['fc', 'inlet', 'fan', 'splitter', 'duct2', 'lpc', 'bld25', 'duct25', 'hpc', 'bld3', 'burner', 'hpt', 'duct45',
        #                     'lpt', 'duct5', 'core_nozz', 'byp_bld', 'duct17', 'byp_nozz', 'gearbox', 'fan_shaft', 'lp_shaft', 'hp_shaft', 'perf',
        #                     'hpt_cooling', 'hpt_chargable', 'balance'])
        # else:
        #     self.set_order(['fc', 'inlet', 'fan', 'splitter', 'duct2', 'lpc', 'bld25', 'duct25', 'hpc', 'bld3', 'burner', 'hpt', 'duct45',
        #                     'lpt', 'duct5', 'core_nozz', 'byp_bld', 'duct17', 'byp_nozz', 'gearbox', 'fan_shaft', 'lp_shaft', 'hp_shaft', 'perf',
        #                     'balance'])
        ...
        if statics:
            ...
            connect_flow(self, 'splitter.Fl_O1', 'duct2.Fl_I')
            connect_flow(self, 'duct2.Fl_O', 'lpc.Fl_I')
            connect_flow(self, 'lpc.Fl_O', 'bld25.Fl_I')
            connect_flow(self, 'bld25.Fl_O', 'duct25.Fl_I')
            connect_flow(self, 'duct25.Fl_O', 'hpc.Fl_I')
            connect_flow(self, 'hpc.Fl_O', 'bld3.Fl_I')
            connect_flow(self, 'bld3.Fl_O', 'burner.Fl_I')
            connect_flow(self, 'burner.Fl_O', 'hpt.Fl_I')
            connect_flow(self, 'hpt.Fl_O', 'duct45.Fl_I')
            connect_flow(self, 'duct45.Fl_O', 'lpt.Fl_I')
            connect_flow(self, 'lpt.Fl_O', 'duct5.Fl_I')
            connect_flow(self, 'duct5.Fl_O','core_nozz.Fl_I')
            connect_flow(self, 'splitter.Fl_O2', 'byp_bld.Fl_I')
            connect_flow(self, 'byp_bld.Fl_O', 'duct17.Fl_I')
            connect_flow(self, 'duct17.Fl_O', 'byp_nozz.Fl_I')
        else:
            connect_flow(self, 'fc.Fl_O', 'inlet.Fl_I', connect_w=False)
            connect_flow(self, 'inlet.Fl_O', 'fan.Fl_I', connect_stat=False)
            connect_flow(self, 'fan.Fl_O', 'splitter.Fl_I', connect_stat=False)
            connect_flow(self, 'splitter.Fl_O1', 'duct2.Fl_I', connect_stat=False)
            connect_flow(self, 'duct2.Fl_O', 'lpc.Fl_I', connect_stat=False)
            connect_flow(self, 'lpc.Fl_O', 'bld25.Fl_I', connect_stat=False)
            connect_flow(self, 'bld25.Fl_O', 'duct25.Fl_I', connect_stat=False)
            connect_flow(self, 'duct25.Fl_O', 'hpc.Fl_I', connect_stat=False)
            connect_flow(self, 'hpc.Fl_O', 'bld3.Fl_I', connect_stat=False)
            connect_flow(self, 'bld3.Fl_O', 'burner.Fl_I', connect_stat=False)
            connect_flow(self, 'burner.Fl_O', 'hpt.Fl_I', connect_stat=False)
            connect_flow(self, 'hpt.Fl_O', 'duct45.Fl_I', connect_stat=False)
            connect_flow(self, 'duct45.Fl_O', 'lpt.Fl_I', connect_stat=False)
            connect_flow(self, 'lpt.Fl_O', 'duct5.Fl_I', connect_stat=False)
            connect_flow(self, 'duct5.Fl_O','core_nozz.Fl_I', connect_stat=False)
            connect_flow(self, 'splitter.Fl_O2', 'byp_bld.Fl_I', connect_stat=False)
            connect_flow(self, 'byp_bld.Fl_O', 'duct17.Fl_I', connect_stat=False)
            connect_flow(self, 'duct17.Fl_O', 'byp_nozz.Fl_I', connect_stat=False)

        connect_flow(self, 'hpc.bld_inlet', 'lpt.bld_inlet', connect_stat=False)
        connect_flow(self, 'hpc.bld_exit', 'lpt.bld_exit', connect_stat=False)
        connect_flow(self, 'bld3.bld_inlet', 'hpt.bld_inlet', connect_stat=False)
        connect_flow(self, 'bld3.bld_exit', 'hpt.bld_exit', connect_stat=False)
...
def viewer(prob, pt, file=sys.stdout): 
    ...
    bleed_names = ['hpc.bld_inlet','hpc.bld_exit','bld3.bld_inlet','bld3.bld_exit','bld25.sbv','hpc.cust']
...
if __name__ == "__main__":
    ...
    # DESIGN CASE
    prob.model.add_subsystem('DESIGN', N3(statics=True))
